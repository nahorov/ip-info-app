...
# Pull the repository
git clone -b master https://github.com/nahorov/ip-info-app.git ~/ip-info-app

# Pull the key-pair file
cp ~/ip-info-app/terraform/20240228.pem ~/.ssh/20240228.pem || { echo "Error: Copying key-pair file failed"; exit 1; }
...
rm -rf ~/ip-info-app/terraform ~/ip-info-app/ip-info-app || { echo "Error: Removing folders failed"; exit 1; }
...
# Run the playbooks
ansible-playbook -i ~/inventory.ini ~/ip-info-app/ansible/java-jenkins-maven/java-jenkins-maven.yml -vvv
ansible-playbook -i ~/inventory.ini ~/ip-info-app/ansible/nexus/nexus.yml -vvv
ansible-playbook -i ~/inventory.ini ~/ip-info-app/ansible/kube-argo-jump/kube-argo-jump.yml -vvv
